				...
				<div className="justify-between mx-auto lg:w-full md:items-center md:flex">
						...
						<div className="flex items-center justify-between py-3 md:py-5 md:block">
							...
							<div className="md:hidden">
								<button className="p-2 text-gray-700 rounded-md outline-none border border-transparent focus:border-gray-400 focus:border" onClick={() => setNavbar(!navbar)}>
									{navbar ? <FaTimes className="text-gray-400 cursor-pointer" size={24} /> : <FaBars className="text-gray-400 cursor-pointer" size={24} />}
								</button>
							</div>
						</div>
					</div>
					...
					<div className={`flex justify-between items-center md:block ${navbar ? "block" : "hidden"}`}>
						<ul className="list-none lg:flex md:flex sm:block block gap-x-5 gap-y-16">
							<li>
								<Link to="/" className="text-gray-400 text-[1.15rem] font-medium tracking-wider hover:text-gray-800 ease-out duration-700">
									الصفحة الرئيسية
								</Link>
							</li>
							<li>
								<Link to="/therapistLists" className="text-gray-400 text-[1.15rem] font-medium tracking-wider hover:text-gray-800 ease-out duration-700">
									الأستشارين
								</Link>
							</li>
							{!token ? (
								<>
									<li>
										{" "}
										<Link to="/SignUp" className="text-gray-400 text-[1.15rem] font-medium tracking-wider hover:text-gray-800 ease-out duration-700">
											اشتراك
										</Link>
									</li>
									<li>
										{" "}
										<Link to="/LogIn" className="text-gray-400 text-[1.15rem] font-medium tracking-wider hover:text-gray-800 ease-out duration-700">
											تسجيل دخول
										</Link>
									</li>
								</>
                 ...
                 gap-2 cursor-pointer p-1
                  hover:bg-light-gray rounded-lg "
                      ...
                      font-bold ml-1 text-14">
										...
										<MdKeyboardArrowDown className=" " onClick={() => setOpenMenu(prev => !prev)} />
									</div>
